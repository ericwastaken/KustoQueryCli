...
The capability manifest is defined in `kusto_query_cli/assets/mcp-manifest.json` inside the application asset directory. It provides machine-discoverable metadata, supported actions, and schema references.

## SECTION 2 - ACTION SCHEMA CONTRACTS
All actions MUST adhere to the JSON Schemas defined in the `kusto_query_cli/assets/schemas/actions/` directory. Each action consists of a request schema and a response data schema.

## SECTION 3 - STANDARD RESPONSE ENVELOPE SCHEMA
Every response from the wrapper MUST be a JSON object validating against `kusto_query_cli/assets/schemas/response-envelope.schema.json`.
...
Clients SHOULD discover capabilities by reading `kusto_query_cli/assets/mcp-manifest.json` or by executing the `MANIFEST` action.
Protocol compatibility MUST be verified using the `kusto_query_cli/assets/mcp-protocol-version` file or the `protocol_version` field in the manifest.
...
- **Semantic Versioning**: The wrapper version follows SemVer (MAJOR.MINOR.PATCH) as defined in the `kusto_query_cli/assets/mcp-wrapper-version` file.
...
Valid interaction examples are provided in the `kusto_query_cli/assets/examples/` directory.
...
- [ ] `MANIFEST` action returns the current state of `kusto_query_cli/assets/mcp-manifest.json`.
