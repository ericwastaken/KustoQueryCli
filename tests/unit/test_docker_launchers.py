...
''')
    ...
    def test_missing_external_image_is_pulled_and_exact_reference_runs(self):
        for script in ("docker/mcp.sh", "docker/cli.sh"):
            with self.subTest(script=script):
                ...
                if script == "docker/mcp.sh":
                    ...
                    self.assertIn(str(ROOT / "docker/compose.yaml"), calls[-1]["args"])
                    self.assertEqual(calls[-1]["args"][calls[-1]["args"].index("--env-file") + 1], str(ROOT / "docker/.env"))
                    self.assertEqual(calls[-1]["args"][calls[-1]["args"].index("--project-directory") + 1], str(ROOT))
    ...
    def test_explicit_image_rejects_force_and_local_build_entrypoint(self):
        for script, args in (
            ("docker/mcp.sh", ["--force"]), ("docker/cli.sh", ["--force", "az", "version"]),
            ("docker/build.sh", []),
        ):
            with self.subTest(script=script):
    ...
    def test_local_missing_image_builds_and_preserves_clean_stdout(self):
        for script in ("docker/mcp.sh", "docker/cli.sh"):
            with self.subTest(script=script):
                ...
                self.assertEqual(result.returncode, 0, result.stderr)
                calls = self.calls()
                self.assertEqual(calls[1]["args"], ["build", "-f", "docker/Dockerfile", "-t", "kusto-query-cli:9.8.7", "."])
                self.assertEqual(result.stdout, "runtime-output\n")
    ...
    def test_cli_down_does_not_acquire_image_and_preserves_exit_status(self):
        result = self.launch("docker/cli.sh", "down", "--volumes", KQC_IMAGE=REMOTE_IMAGE, MOCK_RUN_STATUS="7")
